...
def get_klines():
    # Самый надёжный способ: скачиваем данные с Binance через GitHub Raw (прокси)
    try:
        print("🔄 Загрузка данных...", flush=True)
        url = "https://raw.githubusercontent.com/Edwincripto/btc-bot/main/data/btc_15m.csv"
        df = pd.read_csv(url)
        if not df.empty:
            print(f"✅ Получено {len(df)} свечей", flush=True)
            return df
    except Exception as e:
        print(f"❌ Ошибка загрузки CSV: {e}", flush=True)

    # Запасной вариант: Binance напрямую с другим прокси
    try:
        print("🔄 Пробуем Binance через allorigins...", flush=True)
        proxy_url = "https://api.allorigins.win/raw?url=https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=15m&limit=100"
